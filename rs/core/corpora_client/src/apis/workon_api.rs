...
pub trait WorkonApi: Send + Sync {
    fn file(&self, corpus_file_chat_schema: models::CorpusFileChatSchema) -> Pin<Box<dyn Future<Output = Result<String, Error>> + Send>>;
}
...
impl<C: Connect>WorkonApi for WorkonApiClient<C>
    ...
    #[allow(unused_mut)]
    fn file(&self, corpus_file_chat_schema: models::CorpusFileChatSchema) -> Pin<Box<dyn Future<Output = Result<String, Error>> + Send>> {
        let mut req = __internal_request::Request::new(hyper::Method::POST, "/api/corpora/workon/file".to_string())
        ;
